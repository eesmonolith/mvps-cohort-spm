"""Run multi-lambda transfer on Hospital Billing dataset (K=12)."""
...
def main():
    spec = DATASET_REGISTRY["hospital_billing"]()
    sequences, cohorts, clusters, N_cz = load_dataset(spec)
    K, M = N_cz.shape
    print(f"Hospital Billing: N={len(sequences)}  K={K}  M={M}")
    print("N_cz:\n", N_cz)

    tau_s = 0.7; tau_d = 0.05
    methods = ["freq_only", "stab_only", "discrim_only", "intersect",
               "v1_pooled", "min_ig", "r1_lam50", "r1_lam100", "r1_lam200"]
    ...
    for held_out in range(K):
        ...
        if len(test_seqs) < 10:
            print(f"  fold {held_out}: skipped (size {len(test_seqs)})")
            continue
        ...
        for m, pats in sets.items():
            ...
            agg[m].append(auc)
        el = time.time() - t0
        print(f"  fold {held_out}: freq={agg['freq_only'][-1]:.3f}  "
              f"stab={agg['stab_only'][-1]:.3f}  v1={agg['v1_pooled'][-1]:.3f}  "
    ...
    with open(RESULTS / "hospital_billing_transfer.json", "w") as f:
        json.dump({"K": K, "M": M, "N": len(sequences), "summary": summary,
                   "N_cz": N_cz.tolist()}, f, indent=2)
    print(f"\nwrote {RESULTS / 'hospital_billing_transfer.json'}")
